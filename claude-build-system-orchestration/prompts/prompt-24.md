# Prompt 24: Write phases/01-discover.md
...
Write `C:\Users\Alexb\Documents\Kaitlyn's vs code docs\claude-build-system\phases\01-discover.md` with the following complete content:
...
Next step: Open phases/02-engineer.md. Replace [PROJECT_FOLDER_PATH] with the full path to your [project-name] folder. Paste the full file contents into a fresh chat.
```
...
- [ ] File `C:\Users\Alexb\Documents\Kaitlyn's vs code docs\claude-build-system\phases\01-discover.md` exists
- [ ] The file contains the `AskUserQuestion` tool instruction (confirming questions are not prose)
- [ ] The file does NOT contain the string `[PROJECT_FOLDER_PATH]` anywhere
- [ ] The file contains all 5 question layers (Layer 1 through Layer 5)
- [ ] The file contains the confidence check section and both output file specifications
...
1. Append `"step-24-write-phase-01-discover"` to `completedSteps`
2. Remove `"step-24-write-phase-01-discover"` from `pendingSteps`
3. Append `"claude-build-system/phases/01-discover.md"` to `artifacts.filesWritten`
